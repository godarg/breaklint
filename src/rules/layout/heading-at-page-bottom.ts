...
import {
  ...
  num, outsideContentBoxEvaluation, pageByNumber, renderedBox, sourceOf, startsInContentBox, targetEvaluation,
} from "../shared.ts";
  ...
  (snapshot, ctx) => {
    ...
    for (const block of snapshot.blocks) {
      ...
      const below = onPage.filter((b) => {
        ...
        return other !== null && other.y >= box.y + box.height - 0.5 && startsInContentBox(other, page);
      });
