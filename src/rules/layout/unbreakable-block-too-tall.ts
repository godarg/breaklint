  ...
  (snapshot, ctx) => {
    ...
    for (const block of snapshot.blocks) {
      ...
      if (!hasLayoutBox(block.box)) {
        // ...unless another fragment of the same element does have a box: then the element WAS
        // placed, and excluding its first piece would leave it judged nowhere (its later
        // fragments are not candidates). That is declined instead.
        const siblings = block.sid === null ? [] : fragmentsBySid.get(block.sid) ?? [];
        if (block.fragmentIndex === 0 && siblings.some((fragment) => hasLayoutBox(fragment.box))) {
          candidates += 1;
          decline(block, [
            { name: "fragment-count", value: siblings.length, unit: null, operator: null, threshold: null },
            { name: "target-has-layout-box", value: false, unit: null, operator: "=", threshold: true },
          ]);
          continue;
        }
        evaluations.push(notRenderedEvaluation(RULE, block));
        continue;
      }
