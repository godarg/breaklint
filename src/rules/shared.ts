 ...
 *
 * Pass the block's `renderedBox`, never its raw box: a `display: contents` block has the zero box
 * at the origin, which would pass this test on every page whatever it prints.
 *
 * A block footnote (`float: footnote`) is recorded on the page it is printed on, and the block and
 * line rules measure it like any other block. But Paged.js lays it out in the footnote area, BELOW
 * the content box, which it shortens by the footnote area's height. A rule that asks what follows
 * inside the content box, or that judges a heading as the end of the page's text, must not read a
 * footnote as page text: `layout/heading-at-page-bottom` took a page's footnotes for content
 * following a heading stranded above them, and reported a heading INSIDE a footnote as stranded
