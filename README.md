...
declared peer range) and `pagedjs@0.4.3`. With `pdfjs-dist` installed, the browser must also provide
the JavaScript built-ins and web APIs the pinned rasteriser uses without testing for them; the run
checks them before it opens a document and ends with exit 3 naming any that are missing (see
