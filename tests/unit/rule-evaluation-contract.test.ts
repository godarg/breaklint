...
describe("target evaluation contract", () => {
  ...
  it("bounds a split block by the lines of all its fragments and refuses the ways that can lie", () => {
    ...
    const hiddenFirst = fragmented([{ height: 0, width: 0, x: 0, y: 0 }, ...ofLines(21, 19, 17)]);
    ...
    // The lower bound over the fragments that printed: 21 + 19 + 17 lines, as for `tall` above.
    assert.equal(judgedLater.findings[0]!.measurement.value, 877.77);
    ...
    const invisibleFirst = fragmented(ofLines(10, 21, 19, 17));
    invisibleFirst.blocks[0]!.effectiveStyle = { ...invisibleFirst.blocks[0]!.effectiveStyle, visibility: "hidden" };
    for (const line of invisibleFirst.textLines) if (line.blockKey === invisibleFirst.blocks[0]!.nodeKey) line.visible = false;
