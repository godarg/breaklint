...
- **Snapshot schema 5: every block records its computed `display` and its margin-box copies.**
  (The same stamp also carries `effectiveStyle.columnWidth` and `effectiveStyle.multicolAncestor`
  and populated page withdrawals; see the multi-column and page-withdrawal entries below.)
...
- **New decline reason `env/pagination-residue`, page rows in `documents[].notMeasured`, and two
  ...
  `examples/demo-snapshot.json` and the hand-authored corpus carry both fields explicitly. Both
  are part of Snapshot 5, beside `display` and `marginCopies`.
