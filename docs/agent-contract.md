...
Exit 4 does not require a decline: `--only layout/widow --disable layout/widow` leaves no rule to run and ends exit 4 with "no rule measured a single candidate". A page withdrawn from measurement ends exit 4 on its own, whatever rules are active: today that is a page on which Paged.js left content past the page box, where the PDF does not print it; `documents[].exitReason` names the pages and `documents[].notMeasured[]` carries one `scope: "page"`, `env/pagination-residue` row per page. Where candidates were declined, `documents[].coverage` shows the rule below its floor and `documents[].notMeasured[].reason` names why, as an `env/` id. The two `error` rules require full coverage in the default profile, so one decline by either of them is enough: `layout/unbreakable-block-too-tall` declines on `env/multicolumn` (its own columns, or the columns of a multi-column ancestor), `env/vertical-writing`, `env/pagination-residue` (a fragment on a page withdrawn because content lies past its page box, where the PDF does not print it) or `env/invalid-measurement` (a split block whose fragments cannot be joined by source id, or a printed block with no box of its own), and `svg/text-overflows-viewport` on `env/svg-not-inline`, `env/svg-no-text`, `env/svg-too-many-text-targets`, `env/svg-ctm-unavailable`, `env/svg-viewport-geometry-unsupported` or `env/svg-painted-bounds-unsupported`. Each list is complete for its rule; `tests/unit/agent-contract.test.ts` fails when a rule declares a coverage-relevant reason this page does not list. A target that is out of scope altogether — `env/svg-overflow-visible` — does not count against coverage.
