...
Exit 4 does not require a decline: `--only layout/widow --disable layout/widow` leaves no rule to run and ends exit 4 with "no rule measured a single candidate". Where candidates were declined, `documents[].coverage` shows the rule below its floor and `documents[].notMeasured[].reason` names why, as an `env/` id. The two `error` rules require full coverage in the default profile, so one decline by either of them is enough: `layout/unbreakable-block-too-tall` declines on `env/multicolumn` or `env/vertical-writing`, and `svg/text-overflows-viewport` on `env/svg-ctm-unavailable`, `env/svg-viewport-geometry-unsupported`, `env/svg-painted-bounds-unsupported` or `env/svg-too-many-text-targets`, among the other reasons its rule declares. A target that is out of scope altogether — `env/svg-overflow-visible` — does not count against coverage.
