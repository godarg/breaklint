...
"Text running on" is a text line of a block that continues onto the next page, starting within one of that block's line heights of the top of the next page's content (the higher of its first fill band and its first block that starts there), and not lying — by the centre of its box — inside a block that starts on the next page and follows the continuing block in document order. That last clause is there because a wrapper's lines include its children's: the caption of a carried figure is recorded as the section's line too, and it is the figure's.
