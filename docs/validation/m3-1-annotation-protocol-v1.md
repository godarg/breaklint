...
SVG-source format. This applies to every operational CLI seam: legacy `blind-packet`,
`pipeline-create` and `pipeline-verify` all refuse execution, and the generic file-writing legacy
pipeline module no longer exists. Frozen byte reconstruction remains an in-memory test concern and
cannot itself create or authorize an annotator deliverable.
