...
<!-- breaklint-status-figures-v1 unitTests=615 aggregateTests=746 liveTests=89 liveReportLeaves=226 s1RasterDiffPx=200 s1ForeignRasterDiffPx=200 -->
...
| Geometry cross-check | the product and live oracle share one sampler; selector plus rendered-fragment occurrence binds each in-page box to the exact CDP `DOM.getBoxModel` node. The two agree EXACTLY on this corpus, twice; a systematic 0.002 px disagreement fails the suite. A box the browser split (author columns, Paged.js's overflow column) is compared fragment by fragment against `DOM.getContentQuads`, which equalled `getClientRects()` exactly on 16 fragmented boxes in three documents (patched Chromium 141); its border quad describes such a box as none of its fragments, and comparing the union with it ended ordinary two-column pages in exit 3. A layout change between the two reads still fails the check (`tests/live/measure.test.ts`) |
