...
`breaklint/config.schema.json` and rerun the installed Configuration Contract gate.
...
move nothing that points users at the new tag's Action before `npm view breaklint@X.Y.Z` answers. The later status
