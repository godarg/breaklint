...
For 0.2.3, all of the following must refer to the same commit and the same package bytes:

1. `origin/main` and annotated tag `v0.2.3`;
...
5. npm `breaklint@0.2.3` and its `dist.integrity`;
...
```bash
git tag -a v0.2.3 -m "breaklint 0.2.3"
...
3. creates exactly one `breaklint-0.2.3.tgz`;
...
version, workflow logs and GitHub Release first. If npm already serves 0.2.3 but a post-publish
...
```bash
npm view breaklint@0.2.3 version dist.integrity
npm init -y
npm install breaklint@0.2.3 --no-audit --no-fund
npx breaklint --version
npx breaklint --demo
```

The version must be `0.2.3`; demo must produce real findings and exit 1. Import
